...
TEST_F(LogTest, log_format_empty_string)
{
    const char str[] = "";
...
TEST_F(LogTest, log_format_constant_string)
{
...
TEST_F(LogTest, log_format_non_null_time_less_than_one_day)
{
    const char str[] = "Hello you!";
    set_time(2*3600 + 3*60 + 45);
    buffer = log_format(str, sizeof(str));
...
TEST_F(LogTest, log_format_non_null_time_greater_than_one_day)
{
    const char str[] = "Hello you!";
    set_time(18*24*3600 + 2*3600 + 3*60 + 45);
    buffer = log_format(str, sizeof(str));
...
TEST_F(LogTest, log_format_truncate)
{
    const char str[] = "Hello you!";
...
TEST_F(LogTest, log_and_write)
{
    const char str[] = "Hello you!";
    int fd = get_fake_fd();

    EXPECT_NE(log_and_write(fd, str, sizeof(str)), -1);
    read_equal(fd, str);
    read_equal_name(logfile, "00:00:00 Hello you!\n");
}
